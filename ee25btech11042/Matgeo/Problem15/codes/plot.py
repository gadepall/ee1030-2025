...
lib.tangent_points.argtypes = [ctypes.POINTER(ctypes.c_double), ctypes.c_double,
                               ctypes.POINTER((ctypes.c_double * 2) * 2)]
lib.tangent_points.restype = None

# Input: Circle of radius r, center at origin
r = 4.0
P = (ctypes.c_double * 2)(8.0, 0.0)  # External point P(8,0)

# Array to store solutions (2 tangent points)
sol = ((ctypes.c_double * 2) * 2)()
lib.tangent_points(P, r, sol)

# Extract tangent points from C output
T1 = (sol[0][0], sol[0][1])
T2 = (sol[1][0], sol[1][1])
print(f"Tangent points: T1{T1}, T2{T2}")
...
# Save figure
plt.tight_layout()
fig.savefig(os.path.join(figs_folder, "Figure_1.png"))
plt.show()
