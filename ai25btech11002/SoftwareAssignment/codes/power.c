...
int main() {
    ...
    int k_vals[] = {1, 2, 5, 20, 50, 100, 200, 500};
    ...
    for (int idx_k = 0; idx_k < n_k; idx_k++) {
        ...
        double *V = (double *)malloc(width * k * sizeof(double));
        double *A_copy = (double *)malloc(height * width * sizeof(double));
        for (int i = 0; i < height * width; i++) A_copy[i] = A[i];

        clock_t start = clock();
        svd(A_copy, height, width, k, U, S, V);
        clock_t end = clock();
        double time_spent = (double)(end - start) / CLOCKS_PER_SEC;

        double *Ak = (double *)malloc(height * width * sizeof(double));
        ...
        char out_name[64];
        snprintf(out_name, sizeof(out_name), "recon_%d.pgm", k);
        FILE *out = fopen(out_name, "w");
        fprintf(out, "P2\n%d %d\n255\n", width, height);
        for (int r = 0; r < height; r++) {
            for (int c = 0; c < width; c++) {
                int p = (int)(Ak[r * width + c] * 255.0 + 0.5);
                if (p < 0) p = 0;
                if (p > 255) p = 255;
                fprintf(out, "%d ", p);
            }
        ...
        double orig_size = (double)(height * width);
        double comp_size = (double)k * (height + width + 1);
