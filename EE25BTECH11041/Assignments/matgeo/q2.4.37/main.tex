...
\end{align}
Using Row Transformation $R_2 \rightarrow R_2-2R_1$
\begin{align}
    \vec{A}=\begin{pmatrix} 2&-1&2\\0&1&-1 \end{pmatrix}
\end{align}
Since rank of matrix is 2($\leq 3$), their are infinite many solutions $R^3\rightarrow R^2$\\
Solving equations to get general vectors\\
From the row echelon form,we can write the new system of equation:
\begin{align}
    2x-y+2z=0 \label{3} \\
    y-z=0 \label{4}
\end{align}
Substitute y=z in $\eqref{3}$
\begin{align}
    2x-z+2z=0\\z=-2x
\end{align}
Therefore vector $\vec{C}$ is
\begin{align}
    \vec{C}=(x,-2x,-2x)=x(1,-2,-2)
\end{align}
Now getting vector with magnitude 6
\begin{align}
    |C|=6\\
    |x(1,-2,-2)|=6\\
    |x|\sqrt{(1)^2+(-2)^2+(-2)^2}=6\\
    |x|\sqrt{1+4+4}=6\\
    |x|\sqrt{9}=6\\
    |x|=2
\end{align}
