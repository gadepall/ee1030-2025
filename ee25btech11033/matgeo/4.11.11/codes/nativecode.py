...
# The ratio m:n is given by the first component of the calculated vector.
# Using .2f to format the output nicely.
print(f'Point {tuple(P_dividing)} divides the line AB in the ratio: {ratio[0]:.2f}:1, which simplifies to 13:3')
...
# Updated labels to reflect new coordinates
point_labels = ['A (-2,-5)', 'B (6,3)', 'P (4.5,1.5)']
...
plt.xlabel('$x$')
plt.ylabel('$y$')
# Updated title to reflect new point and calculated ratio
plt.title('Point P(4.5, 1.5) divides AB in ratio of 13:3')
plt.legend(loc='best')
plt.grid(True)
plt.axis('equal')
