...
plane 2 can be written as\\
\begin{align}
    \vec{r}\cdot\vec{n_1}^T+10=0
\end{align}
so the are planes are parllel\\

\end{frame}
\begin{frame}{Theoretical Solution}
Let the distance between planes be K\\
We known that distance between parllel planes is
\begin{align}
K=\frac{\abs{c_1-c_2}}{\|\vec{n_1}\|}=\frac{14}{7}=2
\end{align}
\end{frame}
\begin{frame}[fragile]
    \frametitle{C Code}
